...
cd ../clipper_fastq/
rename 's/\.sra.*_1/_R1/g' *
rename 's/\.sra.*_2/_R2/g' *
...
do
	echo "bwa_${item%.*}"
	bwa mem -t 60 -M -R "@RG\tID:${item%.*}\tSM:${item%.*}\tLB:WES\tPL:Illumina" /proj/y.dong/genome/Homo_sapiens/UCSC/hg38/Sequence/BWAIndex/version0.6.0/genome.fa ./clipper_fastq/${item%.*}_R1.fq ./clipper_fastq/${item%.*}_R2.fq  > ./aligned/${item%.*}.sam 2>> ./aligned/mapping_repo.txt
